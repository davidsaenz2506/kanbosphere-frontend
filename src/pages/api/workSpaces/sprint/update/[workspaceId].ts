...
export default handler.post(async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    const toUpdate: ITransactionSprint = req.body;
    const { workspaceId } = req.query;

    const { data: workspaceUpdated } = await axios.post<IDataToDo>(`${process.env.WORKSPACE_API}/workspaces/updateSprint/${workspaceId}`, toUpdate, {
      headers: {
