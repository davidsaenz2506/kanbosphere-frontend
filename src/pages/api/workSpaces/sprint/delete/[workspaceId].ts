...
export default handler.post(async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    const toDelete: ITransactionSprint = req.body;
    const { workspaceId } = req.query;

    const { data: workspaceUpdated } = await axios.post<IDataToDo>(`${process.env.WORKSPACE_API}/workspaces/deleteSprint/${workspaceId}`, toDelete, {
      headers: {
